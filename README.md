...
4. [Управление системой тестирования](#Управление-системой-тестирования)
...
![Таблица вопросов](/_jpg/27.jpg)

![Таблица вопросов](/_jpg/28.jpg)

На каждый тест создается своя таблица вопросов, например, "test1". На каждую таблицу вопросов создается таблица ответов с добавлением к имени "_a", например, "test1_a". Пользователь, от имени которого осуществляется тестирование должен иметь права "select" на поля таблицы ответов и НЕ ИМЕТЬ права "update".

#### Таблица ответов:
![Таблица ответов](/_jpg/29.jpg)

### Хранимые процедуры
![Хранимые процедуры](/_jpg/5.jpg)

____
[:arrow_up:Оглавление](#Оглавление)
____
## Управление системой тестирования
Доступ к системе управления имеют пользователи из таблицы "24_group". Переход к авторизации осуществляется через скрытую гиперссылку в верхнем левом углу страницы тестирования или по адресу: /test/control.php

![Управление системой тестирования](/_jpg/23.jpg)
![Управление системой тестирования](/_jpg/19.jpg)
![Управление системой тестирования](/_jpg/20.jpg)
![Управление системой тестирования](/_jpg/21.jpg)
![Управление системой тестирования](/_jpg/22.jpg)
...
### Вручную
- Создать таблицу вопросов на основе шаблона `clone_qtest`.
- Заполнить таблицу с помощью `/test/adminer/` или импортировать данные из Excel сторонними продуктами, например, [Navicat](https://www.navicat.com/en/).
- Создать таблицу ответов с добавлением "_a" к имени таблицы вопросов.
- Установить необходимые права на таблицу ответов, выполнив хранимую процедуру `user_grant`. 

### С помощью скрипта на Python
- Заполнить файл Excel по образцу `/_python/test.xlsx`.
- Выполнить скрипт `/_python/mytest.py`
____
[:arrow_up:Оглавление](#Оглавление)
____
## Проблемные вопросы
### 1. При использовании PHP-7.4 и выше модуль управления тестом выдает предупреждение:
`Deprecated: Array and string offset access syntax with curly braces is deprecated in
[путь_к_домашнему_каталогу]\test\class\FastJSON.class.php.`

_Решение:_

а) Использовать PHP-7.2 и ниже.
или:
б) Переписать модуль FastJSON.class.php, заменив {} на [] в нужных местах:), например:
switch($encode{$pos}) заменить на switch($encode[$pos]).

### 2. При тестировании на локальной машине модуль управления тестом выдает ошибку:
`Warning: mysqli::__construct(): (HY000/2002), ссылаясь на
[путь_к_домашнему_каталогу]\test\sec_mod\security_mod.php on line 5.`

_Решение:_

Вместо адреса типа http://localhost/mytest/test/control.php использовать:
http://127.0.0.1/mytest/test/control.php

### 3. При использовании виртуальных хостов при попытке авторизоваться выскакивает ошибка.
![Управление системой тестирования](/_jpg/17.jpg)

_Решение:_

в файле main.php отредактировать ссылку:

`$LoginScript=$_SERVER['HTTP_HOST']."/test/login.php";`

заменить на:

`$LoginScript=$_SERVER['HTTP_HOST']."/mytest/test/login.php";`

где "mytest" - имя виртуального теста.

____
[:arrow_up:Оглавление](#Оглавление)
____
## Состав дистрибутива
- Папки `test` и `test_logs` - АСКУЗ
- Папка `_jpg` - картинки для README.md
- Папка `_python` - программа на Python для добавления таблицы вопросов теста из таблицы Excel
- Папка `_sql` - скрипты sql для развертывания БД `test` в MySQL
